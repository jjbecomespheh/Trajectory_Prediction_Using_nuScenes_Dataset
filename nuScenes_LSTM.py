...
def trainIters(encoder, n_iters, print_every=1000, plot_every=1, learning_rate=0.001):
    ...
    for iter in range(1, n_iters + 1):
        if iter%50==1:
            cp = cp+1
            torch.save(encoder.state_dict(), str(cp)+'nuscenes_checkpoint.pth.tar')
            print("epoch: ", iter)
            inner_loop_count = 0
...
TRAN_TAG = False
if TRAN_TAG:
    if path.exists("nuscenes_checkpoint.pth.tar"):
        Prednet.load_state_dict(torch.load('nuscenes_checkpoint.pth.tar'))
    Prednet = Prednet.double()
    Prednet = Prednet.to(device)
    plot_losses = trainIters(Prednet, 30, print_every=2)
    torch.save(Prednet.state_dict(), 'nuscenes_checkpoint.pth.tar')
